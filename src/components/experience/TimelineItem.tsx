...
const TimelineItem: React.FC<TimelineItemProps> = ({
  ...
  const { title, description, date, color } = work.data;
  ...
  return (
    ...
    >
      <div
        className={`w-5/12 ${
          side === "left"
            ? "text-left pl-4 md:pl-0 md:pr-8"
            : "text-right pr-4 md:pr-0 md:pl-8"
        }`}
      >
      ...
      <div className="w-2/12 flex justify-center relative">
        <TooltipProvider>
          <Tooltip delayDuration={0}>
            <TooltipTrigger>
              <Popover
                onOpenChange={(isOpen) => setIsOpenTooltip(isOpen)}
              >
            ...
            {!isOpenTooltip && (
              <TooltipContent>{"Ver más"}</TooltipContent>
            )}
          </Tooltip>
        </TooltipProvider>
      </div>
